...
<%
	...
	boolean isApproval = request.getAttribute("isApproval") == null ? false : true;
...
jq(function() {
	jq("#list").jqGrid({
		...
		datatype: 'json',
		mtype: 'POST',
		colNames:['승인/반려일','주문일자','납품요청일','주문번호','주문명', '주문유형',  '주문상태', '고객사', '공급사', '공급사 전화번호','주문자', '상품명', '규격','판매단가', '수량', '판매금액', <%if(!isClient){%>'매입단가', '매입금액', <%}%> '긴급여부','disp_good_id', 'vendorid', 'good_iden_numb', 'good_st_spec_desc'],
		colModel:[
			{name:'approvedate',index:'approvedate', width:100,align:"center",search:false,sortable:true, editable:false, editable:false,sorttype:"date", editable:false,formatter:"date"},//주문일자
...
<%	}	%>
		...
		sortname: 'approvedate', sortorder: "desc",
		caption:"주문조회", 
...
function exportExcel() {
	var colLabels = ['승인/반려일','주문일자','납품요청일','주문번호','주문명', '주문유형',  '주문상태', '고객사', '공급사', '공급사 전화번호','주문자', '상품명', '규격','판매단가', '수량', '판매금액', <%if(!isClient){%>'매입단가', '매입금액', <%}%> '긴급여부'];
	var colIds = ['approvedate', 'regi_date_time', 'requ_deli_date', 'orde_iden_numb', 'cons_iden_name', 'orde_type_clas', 'order_status_flag', 'orde_client_name', 'vendor_name', 'phonenum', 'orde_user_name', 'good_iden_name', 'good_spec_desc', 'sell_price', 'orde_requ_quan', 'total_sell_price', <%if(!isClient){%>'sale_unit_pric', 'total_sale_price', <%}%>'emer_orde_clas'];
	var numColIds = ['sell_price','orde_requ_quan','total_sell_price'<%if(!isClient){%>,'sale_unit_pric','total_sale_price'<%}%>];	//숫자표현ID
	var sheetTitle = "주문승인내역조회";	//sheet 타이틀
	var excelFileName = "ApprovalOrderResultList";	//file명
...
<%	if(isApproval){ 	%>
    fieldSearchParamArray[12] = 'srcApproval';
<%	}	%>
<%	if(isClient){ 	%>
    fieldSearchParamArray[13] = 'srcApproveUserId';
<%	}	%>
...
<body>
   ...
   <form id="frm" name="frm">
       <input type="hidden" id="srcApproval" name="srcApproval" value="1"/>
       <input type="hidden" id="srcApproveUserId" name="srcApproveUserId" value="<%=loginUserDto.getUserId()%>"/>
            ...
            <td>
               ...
               <table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr valign="top">
                     ...
                     <td height="29" class='ptitle'>주문승인내역조회</td>
