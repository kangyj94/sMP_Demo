...
$(window).bind('resize', function() {
	...
	$("#list2").setGridWidth($(window).width() - 480 + Number(gridWidthResizePlus));
}).trigger('resize');
</script>
...
function initList() {
	jq("#list").jqGrid({
		...
		datatype:'json',
		mtype:'POST',
		colNames:['사업장명','매출금액','매입금액','손익금액','branchId','매출Id'],
		colModel:[
...
function fnInitProductList(branchid) {
	jq("#list2").jqGrid({
		...
		colModel:[
			{ name:'good_name',index:'good_name',width:180,align:"left",search:false,sortable:true,editable:false },//품목명
			{ name:'vendornm',index:'vendornm',width:150,align:"left",search:false,sortable:true,editable:false },//공급사명
	...
	jq("#list2").jqGrid("setGridParam", { "datatype":"json" });
...
function fnOnClickCustomerProductList(branchid,borgnm) {
	...
	jq("#list2").jqGrid("setCaption", "<font color='blue'>["+borgnm+"]</font>의 상품별 손익실적 (단위 :천원)");
...
function exportExcel() {
	var colLabels = ['사업장명','매출금액','매입금액','손익금액'];	//출력컬럼명
	var colIds = ['borgnm','sale_requ_amou','purc_prod_amou','prof_amou'];	//출력컬럼ID
...
function exportExcel2() {
	var colLabels = ['상품명','공급사명','매출금액','매입금액','손익금액'];	//출력컬럼명
	var colIds = ['good_name','vendornm','sale_prod_amou','purc_prod_amou','prof_amou'];	//출력컬럼ID
	...
	<td>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr valign="top">
				...
				<td height="29" class="ptitle">사업장 손익실적</td>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				<td valign="top" width="610" height="18" align="right">
					<button id='excelButton' class="btn btn-default btn-xs"><i class="fa fa fa-file-excel-o"></i> 엑셀</button>
				</td>
				<td></td>
