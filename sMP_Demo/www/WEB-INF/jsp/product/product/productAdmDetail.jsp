...
<%
    ...
    String reqGoodName = request.getAttribute("reqGoodName") == null ? ""    : (String) request.getAttribute("reqGoodName");
...
*/
%>
<%-- <%  if( good_iden_numb.equals("0") || good_iden_numb.equals("")){ %> --%>
...
<%
		//상품번호는 있지만 카테고리에는 등록안된 품목을 위해 카테고리을 등록해줌
		if( !(good_iden_numb.equals("0") || good_iden_numb.equals(""))){ 
%>
			if(modCategoryFlag && confirm("카테고리정보를 변경하시겠습니까?")){
				$.post(
					"<%=Constances.SYSTEM_CONTEXT_PATH %>/product/updateProductCategory/save.sys",
						{oper:'edit',categortId:categortId, good_iden_numb:'<%=good_iden_numb %>'},
					function(arg){
						var result = eval('('+arg+')').customResponse.success;
						//카테고리정보변경후 히스토리테이블에 insert
						if(result == true){
							$.post(
								"/product/insertMcGoodHist/save.sys",
								{
									oper:'add',
									good_iden_numb:'<%=good_iden_numb %>',
									idGenSvcNm:"seqMcProductHistoryService"
								},
								function(arg){
									if(fnAjaxTransResult(arg)){
										window.opener.jq("#list").trigger("reloadGrid");
									}
								}
							)
						}
...
<script type="text/javascript">
    ...
    function fnInitSalePriceDialog(mod){
        ...
        if(mod == 'add'){
        	...
        	var selrowArray = jq("#prodDisplay").jqGrid('getGridParam','selarrrow');
        	if(selrowArray.length==0) {
        		alert("처리 하실 판가정보를 선택하십시오");
        		return;
        	}
			if(mod == 'mod' && selrowArray.length > 1) {
				alert("수정 하실 판가정보는 한개만 선택 가능합니다.");
        		return;
			}
			...
			if(mod == 'mod') {	//수정처리
	            ...
	            fnProductSalePriceDialog(good_iden_numb,disp_good_id);
			} else if(mod == 'del') {	//삭제처리
				if(!confirm("선택한 진열정보를 삭제 하시겠습니까?")) return false;
				var params = {disp_good_id_array:selrowArray};
                $.post(
...
//     });
	...
	$(document).ready(function() {
		...
		fnLoadDisplayDataSet();
		goodRegYear();
		$('#btnModCategory').click(function () { modCategory();});
    ...
    var fnLoadProductMasterDataSet = function(){
        ...
        jq("#productMaster").jqGrid({
            ...
            colNames:[
                      ...
                      'insert_user_id','insert_date','full_cate_name', 
                      /*  디비 원본 데이터셋 복사본 */
                      'orggood_iden_numb','orgcate_id','orggood_name','orgvtax_clas_code','orgsale_criterion_type',
                      'orgstan_buying_rate','orgstan_buying_money','orgisdistribute','orgisdisppastgood','orginsert_user_id',
                      'orginsert_date', '상품담당자',
                      'is_add_good'
            ],
            colModel:[
                      ...
                      {name:'orggood_iden_numb',index:'orggood_iden_numb',width:100 },            
                      {name:'orgcate_id',index:'orgcate_id',width:100},                           
                      {name:'orggood_name',index:'orggood_name',width:100},                       
                      {name:'orgvtax_clas_code',index:'orgvtax_clas_code',width:100},         
                      {name:'orgsale_criterion_type',index:'orgsale_criterion_type',width:100},   
                      {name:'orgstan_buying_rate',index:'orgstan_buying_rate',width:100},     
                      {name:'orgstan_buying_money',index:'orgstan_buying_money',width:100},       
                      {name:'orgisdistribute',index:'orgisdistribute',width:100},             
                      {name:'orgisdisppastgood',index:'orgisdisppastgood',width:100},         
                      {name:'orginsert_user_id',index:'orginsert_user_id',width:100},         
                      {name:'orginsert_date',index:'orginsert_date',width:100},
                      {name:'product_manager',index:'product_manager',width:100},
                      {name:'is_add_good',index:'is_add_good',width:100}
            ],
            postData: params,
            rownumbers:false,
            height:150,
...
// 					var content = jq("#productMaster").jqGrid('getRowData',selRowId);
// 					alert(content.full_cate_name);
                }else{
                    var addRowDataSet = {
                            good_iden_numb:'0'
                           ,cate_id:'0'
...
//                 FNCopyOrgRowData('productMaster', selRowId , 'org');  
                ...
                //상품등록 요청시 상품명 바인딩 처리
                if(selrowContent.good_name == undefined){
                	var reqGoodName = "<%=reqGoodName%>";
                	if(reqGoodName != ""){
                		$("#good_name").val(reqGoodName);
                		var setRowDataSet = { good_name:reqGoodName};
                    	$("#productMaster").jqGrid('setRowData', 0, setRowDataSet);
                	}
            ...
            afterInsertRow: function(rowid, aData){
                ...
                $("#is_add_good").val(selrowContent.is_add_good);	//추가상품여부
            },
...
//                 var rowCnt = jq("#dispCategory").getGridParam('reccount') ;
//                 for(var ii=0; ii<rowCnt; ii++) {
//                     var rowid = $("#dispCategory").getDataIDs()[ii];
//                     //var rowData = jq("#dispCategory").jqGrid('getRowData',rowid);
                    
//                     var is_Disp_Use_Kr = ''; 
//                     if(FNgetGridDataObj('dispCategory' , rowid , 'is_Disp_Use') == '0') is_Disp_Use_Kr = '미사용';  
//                     else is_Disp_Use_Kr = '사용';
                    
//                     // 수정여부 컬럼에 데이터 설정 
//                     FNAlterGridDataProValue('dispCategory' , rowid , 'is_Disp_Use' , is_Disp_Use_Kr);
//                 }
            },
            onSelectRow:function(rowid,iRow,iCol,e) {},
            ondblClickRow:function(rowid,iRow,iCol,e) {},
	...
	var fnLoadProductVendorDataSet = function(){
		...
		jq("#prodVendor").jqGrid({
			...
			colNames:[
					...
					/* 복사본 원본 데이터 */
					'원본상품코드', '원본공급사ID', '원본공급사코드', '원본공급사명', '원본대표자명', 
					'원본소재지', '원본대표연락처', '원본상품등록요청SEQ', '원본매입단가','distri_rate' , 
					'원본규격', '원본표준규격', '원본규격1','원본규격2','원본규격3',
					'원본규격4','원본규격5','원본규격6','원본규격7','원본규격8',
					'원본표준규격1','원본표준규격2','원본표준규격3','원본표준규격4','원본표준규격5',
					'원본표준규격6', '원본단위', '원본납품소요일수', '원본최소구매수량', '원본제조사', 
					'원본이미지등록여부', '원본대표이미지원본', '원본대표이미지대', '원본대표이미지중', '원본대표이미지소', 
					'원본동의어', '원본동의어1','원본동의어2','원본동의어3','원본동의어4',
					'원본동의어5', '원본상품설명등록여부', '원본상품설명', '원본하도급법대상여부','원본상품구분',
					'원본재고수량','원본등록일','원본bidid','원본bid_vendorid' , 'orgreq_good_id' ,
					'prodvendorappflag' ,'chageProductRequest','isUse','상품실적년도','상품우선순위','당일발송'
			],
			colModel:[
					/*수정모드*/      
					...
					/*복사본 디비 원본  데이터*/
					{name:'orggood_iden_numb',index:'orgood_iden_numb',hidden:true,sortable:false},   /*원본상품코드*/
					{name:'orgvendorid',index:'orvendorid',hidden:false,sortable:false},               /*공급사ID*/
					{name:'orgvendorcd',index:'orvendorcd',hidden:true,sortable:false},               /*공급사코드*/
					{name:'orgvendornm',index:'orvendornm',hidden:true,sortable:false},               /*공급사명O*/
					{name:'orgpressentnm',index:'orpressentnm',hidden:true,sortable:false},           /*대표자명O*/
					{name:'orgphonenum',index:'orphonenum',width:80,hidden:true,sortable:false},      /*소재지O*/
					{name:'orgareatype',index:'orareatype',hidden:true,sortable:false},               /*대표연락처O*/
					{name:'orgreq_good_id',index:'orreq_good_id',hidden:true,sortable:false},         /*상품등록요청SEQ*/
					{name:'orgsale_unit_pric',index:'orsale_unit_pric',hidden:true,sortable:false},   /*매입단가O*/
					{name:'orgdistri_rate',index:'orgdistri_rate',hidden:true,sortable:false},        /*원본배분율 O*/
					{name:'orggood_spec_desc',index:'orgood_spec_desc',hidden:true,sortable:false},   /*규격*/
					{name:'orggood_st_spec_desc',index:'orggood_st_spec_desc',hidden:true,sortable:false},   /*규격*/
					{name:'orggood_spec_desc1',index:'orggood_spec_desc1',hidden:true,sortable:false},/*규격1*/
					{name:'orggood_spec_desc2',index:'orggood_spec_desc2',hidden:true},/*규격2*/
					{name:'orggood_spec_desc3',index:'orggood_spec_desc3',hidden:true},/*규격3*/
					{name:'orggood_spec_desc4',index:'orggood_spec_desc4',hidden:true},/*규격4*/
					{name:'orggood_spec_desc5',index:'orggood_spec_desc5',hidden:true},/*규격5*/
					{name:'orggood_spec_desc6',index:'orggood_spec_desc6',hidden:true},/*규격6*/
					{name:'orggood_spec_desc7',index:'orggood_spec_desc7',hidden:true},/*규격7*/
					{name:'orggood_spec_desc8',index:'orggood_spec_desc8',hidden:true},/*규격8*/
					{name:'orggood_st_spec_desc1',index:'orggood_st_spec_desc1',hidden:true,sortable:false},         /*표준규격1*/
					{name:'orggood_st_spec_desc2',index:'orggood_st_spec_desc2',hidden:true,sortable:false},         /*표준규격2*/
					{name:'orggood_st_spec_desc3',index:'orggood_st_spec_desc3',hidden:true,sortable:false},         /*표준규격3*/
					{name:'orggood_st_spec_desc4',index:'orggood_st_spec_desc4',hidden:true,sortable:false},         /*표준규격4*/
					{name:'orggood_st_spec_desc5',index:'orggood_st_spec_desc5',hidden:true,sortable:false},         /*표준규격5*/
					{name:'orggood_st_spec_desc6',index:'orggood_st_spec_desc6',hidden:true,sortable:false},         /*표준규격6*/
					{name:'orgorde_clas_code',index:'ororde_clas_code',hidden:true},   /*단위*/
					{name:'orgdeli_mini_day',index:'ordeli_mini_day',hidden:true},     /*납품소요일수*/
					{name:'orgdeli_mini_quan',index:'ordeli_mini_quan',hidden:true},   /*최소구매수량*/
					{name:'orgmake_comp_name',index:'ormake_comp_name',hidden:true},   /*제조사*/
					{name:'orgisexistimg',index:'orgisexistimg',hidden:true},          /*이미지등록여부O*/
					{name:'orgoriginal_img_path',index:'ororiginal_img_path',hidden:true},/*대표이미지원본*/
					{name:'orglarge_img_path',index:'orlarge_img_path',hidden:true},   /*대표이미지대*/
					{name:'orgmiddle_img_path',index:'ormiddle_img_path',hidden:true}, /*대표이미지중*/
					{name:'orgsmall_img_path',index:'orsmall_img_path',hidden:true},   /*대표이미지소*/
					{name:'orggood_same_word',index:'orgood_same_word',hidden:true},   /*동의어*/
					{name:'orggood_same_word1',index:'orggood_same_word1',hidden:true},/*동의어*/
					{name:'orggood_same_word2',index:'orggood_same_word2',hidden:true},/*동의어*/
					{name:'orggood_same_word3',index:'orggood_same_word3',hidden:true},/*동의어*/
					{name:'orggood_same_word4',index:'orggood_same_word4',hidden:true},/*동의어*/
					{name:'orggood_same_word5',index:'orggood_same_word5',hidden:true},/*동의어*/
					{name:'orgisexistgooddesc',index:'orisexistgooddesc',hidden:true}, /*상품설명등록여부O*/
					{name:'orggood_desc',index:'orgood_desc',hidden:true},             /*상품설명*/
					{name:'orgissub_ontract',index:'orissub_ontract',hidden:true},     /*하도급법대상여부*/
					{name:'orggood_clas_code',index:'orgood_clas_code',hidden:true},   /*상품구분*/
					{name:'orggood_inventory_cnt',index:'orgood_inventory_cnt',hidden:true},/*재고수량*/
					{name:'orgregist_date',index:'orregist_date',hidden:true},         /*등록일O*/            
					{name:'orgbidid',index:'orgbidid',align:"center",width:95,hidden:true},             /*입찰 번호 O*/
					{name:'orgbid_vendorid',index:'orgbid_vendorid',align:"center",width:95,hidden:true},           /*입찰공급사 O*/
					{name:'orgreq_good_id',index:'orgreq_good_id',align:"center",width:95,hidden:true},             /*고객사상품등록요청  O*/
					{name:'orgprodvendorappflag',index:'orgprodvendorappflag',align:"center",width:95,hidden:true},  /*  승인여부(종료,매입가변경)    */
					{name:'orgchageproductrequest',index:'orgchageproductrequest',align:"center",width:95,hidden:true},  /*  요청여부(종료,매입가변경)    */
					{name:'orgisUse',index:'orgisUse',align:"center",width:95,hidden:true},             /*정상여부 O*/
					{name:'good_reg_year',index:'good_reg_year',align:"center",width:95,hidden:true},             /*상품실적년도*/
					{name:'vendor_priority',index:'vendor_priority',align:"center",width:95,hidden:true},            /*상품우선순위*/
					{name:'the_day_post',index:'the_day_post',align:"center",width:95,hidden:false}             /*당일발송*/
			],
			gridview:true,
			postData:params,
			rowNum:0,rownumbers:false,
...
//                     // 해당 로우에 원본데이터 복사 
// //                     FNCopyOrgRowData('prodVendor', rowid , 'org');  
                    
//                     // 수정여부 컬럼에 데이터 설정 
//                     FNAlterGridDataProValue('prodVendor' , rowid , 'isModify' , '0');
                    
//                     // 종료 여부 설정  0 종료 , 1이면 정상 
//                     var isUseText = '';
//                     if(FNgetGridDataObj('prodVendor' , rowid , 'isUse') == '0')     isUseText= '종료'; 
//                     FNAlterGridDataProValue('prodVendor' , rowid , 'isUse' , isUseText);
//                 }
				
				// 공급사 정보 Parameter 존재시 해당 그리드를 선택 한다. 
				if($.trim(reqParamObj['vendorid']).length > 0 ) 
					jQuery("#prodVendor").setSelection(reqParamObj['vendorid']);
                //vendorDef.notify('loadPrdVendorDataDone');
//              //----------------------------------------------------------------------------------------
//              // 입찰에서 호출한 경우 입찰 품목 정보를 존재한후 해당 로우를 생성해준다.   
//              //----------------------------------------------------------------------------------------
//              fnLoadBidVendorInfo();
...
//                 fnSetProdVendorComponent();
			...
			onSelectRow:function(rowid,iRow,iCol,e) {
				...
				//상품실적년도 세팅
				var getYear = <%=getYear%>
				var selrowContent = $("#prodVendor").jqGrid('getRowData',rowid);
				var year = selrowContent.good_reg_year;
				var theDayPost = selrowContent.the_day_post
				if(year != '' && year != null){
					$("#good_reg_year").val(year);
				}
				if(theDayPost != '' && theDayPost != null){
					$("#the_day_post").val(theDayPost);
				}
			},
			ondblClickRow:function(rowid,iRow,iCol,e) {},
			onCellSelect:function(rowid,iCol,cellcontent,target) {
...
//                     }
                    ...
                    $('#prodVendor').jqGrid('setRowData',addRowData['vendorid'],{vendor_priority:0});
                }
            );    
        }
    };
...
<script type="text/javascript">
    ...
    var fnChangeCriterion = function() {
        ...
        //상품등록요청 후 상픔 등록시 상품등록하는 사람의 세션아이디를 상품담당자에 배정
        var req_good_id = "<%=req_good_id%>";
//         alert(req_good_id);
		if(req_good_id != ""){
			var productManagerId = "<%=loginUserDto.getUserId()%>";
			$("#product_manager").val(productManagerId);
			if($("#product_manager").val() != ""){
				$("#productMaster").jqGrid('setRowData', 0, {product_manager:productManagerId});
			}
		}
        if(val=='10') {   //기준매익률
        ...
        jQuery('#productMaster').jqGrid('setRowData',rowId,{isModify:1});
    };
	...
	var fnSetProdVendorComponent = function() {
		...
		if(selrow != null){
			...
			if( rowData['regist_date'] != '' ){
				if($.trim(rowData['orgisUse']) == '0' || $.trim(rowData['orgisUse']) == '2') 	$('#btnChnUseSts').show();
			...
			//상품등록요청시 우선순위 값이 없으므로 디폴트 값 0을 넣음
			if($.trim(rowData['vendor_priority']) == ''){
				$('#vendor_priority').val(0);
			}
		}
	};
...
//      }
    ...
    var fnProdVendorCopy = function (){
        ...
        var copyColNms = new Array("sale_unit_pric","good_spec_desc","orde_clas_code","deli_mini_day","deli_mini_quan","make_comp_name","isexistimg","original_img_path","large_img_path","middle_img_path","small_img_path","good_same_word","isexistgooddesc","good_desc","issub_ontract","good_clas_code","good_reg_year","vendor_priority","the_day_post","good_st_spec_desc");
    ...
    var fnProdVendorAdd = function(){
        var newRowId = $("#prodVendor").getDataIDs().length+1;
        jQuery("#prodVendor").addRowData(newRowId, {issub_ontract:'0',good_clas_code:'10',deli_mini_quan:'1',deli_mini_day:'3',good_reg_year:'',vendor_priority:0, the_day_post:0});
                                                    //issub_ontract
        jQuery("#prodVendor").setSelection(newRowId);
        fnSetProductDescContents(Editor.getContent());
        fnSetProdVendorComponent();
	...
	var  fnSaveProductInfo = function(){
		...
		*/
		...
		$.post(
			"/productManage/prodyctInfoTransGrid.sys", 
			params,
			function(arg){ 
				if(fnTransResult(arg, false) ){
					var msg = eval('(' + arg + ')').msg;
					var masSuccess = eval('(' + arg + ')').masSuccess;
...
// 						alert(req_good_id);
// 						if(req_good_id != ""){
// 							$(opener.location).attr("href", "javascript:fnReLoadDataGrid();");
// // 							window,close();
// 						}
...
//         }
    ...
    var fnCheakValidationProdInfo = function(){
        ...
        //상품담당자
        if( $.trim(mstGridData['product_manager']) == ''){
        	$('#dialogSelectRow').html('<p>상품담당자를 선택해 주세요.</p>');
            $("#isdistribute").focus();
            $("#dialogSelectRow").dialog();
            return false;
        }
...
//                 }
                ...
                // 단위
                if($.trim(venRowData["orde_clas_code"]) == '' || $.trim(venRowData["orde_clas_code"]) == '선택') {
             ...
             // 상품실적년도
                if($.trim(venRowData["good_reg_year"]) == '' && $.trim(venRowData["orgisUse"]) == '1') {
...
//                 }
                //vendor_priority
                if($.trim(venRowData["vendor_priority"]) == '') {
                    $('#dialogSelectRow').html('<p>' +parseInt(i+1)+ '번째 상품공급사의 상품우선순위가 존재하지 않습니다.</p>');
                    $("#dialogSelectRow").dialog();
                    return false; 
                }
...
//         stan_buying_money
//         if(parseInt($('#price').val().replace(/,/g , "")) > parseInt($('#stan_buying_money').val().replace(/,/g , ""))){
//             $('#dialogSelectRow').html('<p>변경매입단가는 절대판매가보다 큰 값을 입력할수 없습니다. </p>');
//             $("#dialogSelectRow").dialog();
//             return false; 
//         }
    ...
    */
    var fnUseStatsModify = function(){
		...
		// 선택된 공급사 존재시 
		if(rowId != null){
			var rowData = jQuery("#prodVendor").jqGrid('getRowData',rowId);
		}
		
        if($.trim(rowData['orgisUse']) == '0'){
        	statStr = "종료";
        }
        else if($.trim(rowData['orgisUse']) == '2'){
        	statStr = "대기";
        }
        
        if(!confirm(statStr+ "상태를 변경 하시겠습니까?")) 	return false;
...
<script type="text/javascript">
    ...
    function fnVendorExcell(){
        var colLabels = ['공급사명','대표자명','대표연락처','소재지','매입단가','납품소요일수','최소구매수량','이미지등록여부','상품설명등록여부','등록일','상품실적년도'];   //출력컬럼명
        var colIds = ['vendornm','pressentnm','phonenum','areatype','sale_unit_pric','deli_mini_day','deli_mini_quan','isexistimg','isexistgooddesc','regist_date','good_reg_year'];    //출력컬럼ID
    ...
    function fnDispExcell(){
        var colLabels = ['소재지','진열고객사명','고객사 상품코드','판매가','매입가','공급사명','계약시작일','계약종료일'];   //출력컬럼명
        var colIds = ['areanm','fullborgnms','cust_good_iden_numb','sell_price','sale_unit_pric','vendornm','cont_from_date','cont_to_date'];    //출력컬럼ID
...
//         if(goodRegYearCnt == 0){
        	for(var i=getYear; i>=2010;i--){
            	$("#good_reg_year").append("<option value="+i+">"+i+"년</option>");
            }
//         }
// 		var rowCnt = $("#prodVendor").getGridParam('reccount');
// 		var year = "";
// 		if(rowCnt > 0){
// 			for(var i=0; i<rowCnt; i++){
// 				var rowid = $('#prodVendor').getDataIDs();
// 				var selrowContent = $("#prodVendor").jqGrid('getRowData',rowid);
// 				year = selrowContent.good_reg_year;
// 			}
// 			alert(rowid);
// 			if(year == "" || year == null){
// 				$("#good_reg_year").val(getDate).attr("selected", "selected");
// 			}else{
// 				$("#good_reg_year").val(year).attr("selected", "selected");
// 			}
// 		}
	}

	//카테고리변경
	var modCategoryFlag = false;
	function modCategory() {
		modCategoryFlag = true;
		fnSearchCategory();
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						...
						<tr>
							...
							<td class="table_td_contents">
								...
								<img id='btnModCategory' src="<%=Constances.SYSTEM_IMAGE_URL%>/img/system/btn_type3_modCategory.jpg" width="86" height="18" class='icon_search' style="vertical-align: middle; cursor: pointer" />
							</td>
						</tr>
						<tr>
							<td colspan="2" class="table_top_line"></td>
				...
				<td>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						...
						<tr>
							...
							<td class="table_td_contents">
								<input id="good_name" name="good_name" type="text" maxlength="50" value="" onchange="javascript:fnSetChangeComponentData(this,event,'productMaster','org');" />
							</td>
						...
						<tr>
							...
							<td class="table_td_contents">
								<input id="stan_buying_rate" name="stan_buying_rate" alt='persent' type="text" value="" size="5" maxlength="5" style="text-align: right;" onchange="javascript:fnSetChangeComponentData(this,event,'productMaster','org');" />%
								<!-- onblur="javascript:fnSetFormatCurrency(this);" -->
							</td>
							<td class="table_td_subject">절대판매가</td>
							<td class="table_td_contents">
								<input id="stan_buying_money" name="stan_buying_money" alt='number' type="text" value="" size="20" maxlength="10" style="text-align: right;" onchange="javascript:fnSetChangeComponentData(this,event,'productMaster','org');" onfocus="javascript:fnSetFormatCurrency(this);" onblur="javascript:fnSetFormatCurrency(this);" onkeydown="javaScript:return onlyNumber(event);"/>
						...
						<tr>
							<td class="table_td_subject9">추가구성상품여부</td>
							<td class="table_td_contents" colspan='5'>
								<select id="is_add_good" name="is_add_good" class="select" onchange="javascript:fnSetChangeComponentData(this,event,'productMaster','org');">
									<option value="0">아니오</option>
									<option value="1">예</option>
								</select>
							</td>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						...
						<tr>
							<td class="table_td_subject9" width="100">공급사</td>
								<td class="table_td_contents">
									<input id="vendornm" name="vendornm" type="text" value="" size="" maxlength="50" style="width: 120px; background-color: #eaeaea;" disabled="disabled" onchange="javascript:fnSetChangeComponentData(this,event,'prodVendor','org');" />
									<input id="vendorid" name="vendorid" type="text" style="visibility: hidden; width: 0px;" value="" onchange="javascript:fnSetChangeComponentData(this,event,'prodVendor','org');" />
							...
							<tr>
								...
								<td class="table_td_contents">
									<input id="sale_unit_pric" name="sale_unit_pric" alt='number' type="text" value="" size="15" maxlength="10" style="text-align: right;" onchange="javascript:fnSetChangeComponentData(this,event,'prodVendor','org');" onblur="javascript:fnSetFormatCurrency(this);" onkeydown="return onlyNumber(event);" />
								...
								<td class="table_td_contents">
									<input id="deli_mini_quan" name="deli_mini_quan" alt='number' type="text" value="" size="5" maxlength="4" style="text-align: right;" onchange="javascript:fnSetChangeComponentData(this,event,'prodVendor','org');" onblur="javascript:fnSetFormatCurrency(this);" onkeydown="return onlyNumber(event);" />
								</td>
							</tr>
							<tr>
								<td colspan="6" height="1" bgcolor="eaeaea"></td>
...
<!--                                 </td> -->
								...
								<td class="table_td_contents">
									<select id="good_reg_year" name="good_reg_year" onchange="javascript:fnSetChangeComponentData(this,event,'prodVendor','org');" class="select">
										<option value="">선택</option>
									</select>
								</td>
							</tr>
							...
							<tr>
								<td class="table_td_subject9">납품소요일</td>
								<td class="table_td_contents">
									<input id="deli_mini_day" name="deli_mini_day" alt='number' type="text" value="" size="4" maxlength="2" style="text-align: right;" onchange="javascript:fnSetChangeComponentData(this,event,'prodVendor','org');" onblur="javascript:fnSetFormatCurrency(this);" onkeydown="return onlyNumber(event);" />일
								</td>
								...
								<td class="table_td_contents">
									<input id="good_inventory_cnt" name="good_inventory_cnt" alt='number' type="text" value="" size="9" maxlength="7" style="text-align: right;"
							...
							<tr>
								<td class="table_td_subject">상품우선순위</td>
								<td class="table_td_contents">
									<input id="vendor_priority" name="vendor_priority" type="text" alt='number' value="" size="7" maxlength="50" onkeydown="return onlyNumber(event);"
										onblur="javascript:fnSetFormatCurrency(this);"
										onchange="javascript:fnSetChangeComponentData(this,event,'prodVendor','org');"/>&nbsp;&nbsp;
										<font color="red">*</font>&nbsp;숫자가 커야 우선 순위가 높게 적용됩니다.
								</td>
								<td class="table_td_subject">당일발송</td>
								<td class="table_td_contents">
									<select id="the_day_post" name="the_day_post" onchange="javascript:fnSetChangeComponentData(this,event,'prodVendor','org');" class="select">
										<option value="0">아니오</option>
										<option value="1">예</option>
									</select>
								</td>
                            ...
                            <td bgcolor="#FFFFFF">
                                ...
                                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                    ...
                                    <tr>
                                        ...
                                        <td class="table_td_contents">
                                            <input id="price" name="price" alt='number' type="text" value="" size="10" maxlength="10" style="text-align: right;" onchange="javascript:fnSetChangeComponentData(this,event,'prodVendor','org');" onblur="javascript:fnSetFormatCurrency(this);" onkeydown="return onlyNumber(event);" />
                                        </td>
                                    </tr>
                                    <tr style="height: 1px" >
                                        <td></td>
