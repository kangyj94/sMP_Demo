	...
	function checkDragAndDropAPI(){
		//bSupportDragAndDropAPI = false : 단일 파일업로드만 하기때문에 
		try{
//			if( !oNavigator.ie ){
	 ...
	 */
     function goStartMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") < 0 ){
    		 welBtnConfirm.attr("src","/SmartEditor/sample/photo_uploader/img/btn_confirm2.png");
     ...
     function goReadyMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") >= 0 ){
	    	 ...
	    	 welBtnConfirm.attr("src","/SmartEditor/sample/photo_uploader/img/btn_confirm.png");
    	 }
     }   
...
// 	        sCallback : location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 			sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/nseiu.sys',	//Naver Smart Editor Image Upload
 	        sCallback : location.href.replace(/\/[^\/]*$/, '') + '/imgUploadCallback.sys',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
