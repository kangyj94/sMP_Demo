...
jq(function() {
	jq("#list").jqGrid({
		...
		datatype: 'json',
		mtype: 'POST',
		colNames:['승인/반려일','주문일자','납품요청일','주문번호','공사명', '주문유형',  '주문상태', '고객사', '공급사', '공급사 전화번호','주문자', '상품명', '규격','판매단가', '수량', '판매금액', <%if(!isClient){%>'매입단가', '매입금액', <%}%> '긴급여부','disp_good_id', 'vendorid', 'good_iden_numb', 'good_st_spec_desc'],
		colModel:[
...
function exportExcel() {
	var colLabels = ['승인/반려일','주문일자','납품요청일','주문번호','공사명', '주문유형',  '주문상태', '고객사', '공급사', '공급사 전화번호','주문자', '상품명', '규격','판매단가', '수량', '판매금액', <%if(!isClient){%>'매입단가', '매입금액', <%}%> '긴급여부'];
