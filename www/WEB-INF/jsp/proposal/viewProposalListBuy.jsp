...
$(document).ready(function() {
	...
	$("#srcSuggestTitle").keydown(function(e){ if(e.keCode == 13){ $("#srcButton").click(); } });
		...
		function(arg){
			...
			if(records > 0){
				...
				for(var i=0; i<list.length; i++){
					...
					str +=		'</td>';
					str +=		'<td align="center" '+ ( fnNullToSpace( data.ACCEPTDATE ) ? bgColor : "" )+ '>';
					str +=			'<p>'+ fnNullToSpace( data.ACCEPTDATE )+'</p>';
					str +=			'<p>'+ fnNullToSpace( data.ACCEPT_USER_NM )+'</p>';
					str +=		'</td>';
					str +=		'<td align="center" '+ ( fnNullToSpace( data.SUITABLEDATE ) ? bgColor : "" )+ '>';
					str +=			'<p>'+ fnNullToSpace( data.SUITABLEDATE )+'</p>';
					str +=			'<p>'+ fnNullToSpace( data.SUITABLE_USER_NM )+'</p>';
					str +=		'</td>';
					str +=		'<td align="center" '+ ( fnNullToSpace( data.APPRAISALDATE ) ? bgColor : "" )+ '>';
...
%>
				...
				<div class="mgt_10">
					<p id="proposalCntText"></p>
					<span style="position:absolute; right:0; display:block; margin-top:-20px;">
						<button id='regiBtn' class="btn btn-darkgray btn-xs" ><i class="fa fa-plus"></i> 제안하기</button>
					</span>
				</div>
