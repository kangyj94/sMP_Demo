...
<%
	...
	String boardTitle = "";
	if("0203".equals(board_Type)) boardTitle = "벼룩시장[판매]";
	else if("0204".equals(board_Type)) boardTitle = "벼룩시장[구매]";
...
jq(function() {
	jq("#list").jqGrid({
		url:'<%=Constances.SYSTEM_CONTEXT_PATH%>/board/fleaMarketListJQGrid.sys',
		datatype: 'json',
		mtype: 'POST',
		colNames:['번호','제목','첨부갯수','등록일','작성자','조회수','Lev'],
		colModel:[
		...
		ondblClickRow: function (rowid, iRow, iCol, e) {
			...
			if (selrowContent.regi_User_Numb == "<%=loginUserDto.getUserNm()%>") {			
...
function viewRow() {
	...
	if( row != null ){
		...
		var popurl = "/board/fleaMarketDetail.sys?board_No=" + selrowContent.board_No;
...
function regist(){
	var popurl = "/board/fleaMarketWrite.sys?board_Type=<%=board_Type%>";
	var popproperty = "dialogWidth:720px;dialogHeight=700px;scroll=auto;status=no;resizable=no;";
// 	window.showModalDialog(popurl,self,popproperty);
	window.open(popurl, 'okplazaPop', 'width=720, height=700, scrollbars=yes, status=no, resizable=no');
}
...
function modify(){
	...
	if( row != null ){
		var selrowContent = jq("#list").jqGrid('getRowData',row);        // 선택된 로우의 데이터 객체 조회
		var popurl = "/board/fleaMarketWrite.sys?board_No=" + selrowContent.board_No;
		var popproperty = "dialogWidth:720px;dialogHeight=700px;scroll=auto;status=no;resizable=no;";
// 		window.showModalDialog(popurl,self,popproperty);
		window.open(popurl, 'okplazaPop', 'width=720, height=700, scrollbars=yes, status=no, resizable=no');
	} else { jq( "#dialogSelectRow" ).dialog(); }
...
function deleteRow() {
	...
	if( row != null ) {
		jq("#list").jqGrid( 
			'delGridRow', row,{
				url:"<%=Constances.SYSTEM_CONTEXT_PATH%>/board/fleaMarketTransGrid.sys",
...
function exportExcel() {
	...
	var sheetTitle = "벼룩시장[판매]";	//sheet 타이틀
	var excelFileName = "fleaMarkgetList";	//file명
...
<% if(board_Type.equals("0203")){ %>
					<td height="29" class='ptitle'>벼룩시장[판매]</td>
<% } else if(board_Type.equals("0204")) { %>
					<td height="29" class='ptitle'>벼룩시장[구매]</td>
<% }%>
					<td align="right">
						<a href="#">
			...
			<table width="100%"  border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td align="right" valign="bottom">
						...
						<a href="#"><img id="regButton" src="<%=Constances.SYSTEM_IMAGE_URL%>/img/system/icon/Add.gif" width="15" height="15" style='border:0;<%=CommonUtils.getDisplayRoleButton(roleList, "COMM_READ")%>' /></a>
