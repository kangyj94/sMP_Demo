...
$(function(){
	...
	new AjaxUpload(btnUpload, {
		action: '<%=Constances.SYSTEM_CONTEXT_PATH%>/common/attachFileUpload.sys',
		name: 'uploadFile',
		data: {},
...
var fnCallback = "";
function fnUploadDialog(attach_title, attach_seq, callbackString,chkSize) {
	$('#uploadDialogPop').jqmShow();
	$("#attach_seq").val(attach_seq);
	$('#attach_title').text(attach_title);
	$('#status').text("");
	$("#chkSize").val(chkSize);
	$("#chkSizeText").val(chkSize);
...
<input type="hidden" id="chkSize" name="chkSize" />
<div class="jqmWindow1" id="uploadDialogPop">
				...
				<div id="uploadDialogHandle">
					<table id="popup_titlebar_mid1" width="100%"  border="0" cellpadding="0" cellspacing="0" style="background-image: url('<%=Constances.SYSTEM_IMAGE_URL%>/img/system/popup_titlebar_mid.gif');">
		      			<tr>
		        			...
		        			<td style="background-color: #ea002c; height: 47px;color: #fff;font-size: 14px;font-weight: 700;">PPT파일</td>
					...
					<tr>
						...
						<td bgcolor="#FFFFFF">
							...
							<table width="100%"  border="0" cellspacing="0" cellpadding="0">
								...
								<tr valign="top">
									<td height="23px" class="table_td_contents">3. 파일은 <input type="text" id="chkSizeText" name="chkSizeText" style="border: 0px;width: 10px; text-align: right;" readonly="readonly"/>메가바이트 이상 등록할 수 없습니다.</td>
								</tr>
								<tr><td>&nbsp;</td></tr>
							</table>
							...
							<table width="100%"  border="0" cellspacing="0" cellpadding="0">
								<tr>
									<td align="center">
										<button id='attachImportButton' class="btn btn-darkgray btn-sm"> 등록</button>
										<button id='attachCloseButton' class="btn btn-default btn-sm" ></i> 닫기</button>
<%-- 										<a href="#"><img id="attachImportButton" src="<%=Constances.SYSTEM_IMAGE_URL%>/img/system/btn_type3_fileReg.gif" style='border:0;' /></a> --%>
<%-- 										<a href="#"><img id="attachCloseButton" src="<%=Constances.SYSTEM_IMAGE_URL%>/img/system/btn_type3_close.gif" style='border:0;' /></a> --%>
									</td>
								</tr>
							</table>
						</td>
