...
//상품 마스터의 주문단위와 상품 요청의 주문단위의 명칭이 달라서 통일하기 위해서 작업
String GOOD_UNIT = "";

if (good != null){
	GOOD_UNIT = (String)good.get("ORDER_UNIT");

	if (GOOD_UNIT == null){
		GOOD_UNIT = (String)good.get("ORDE_CLAS_CODE");
	}
}
%>
<head>
...
<div id="divPopup" style="width:900px;">
	<div class="popcont">
		...
		<form id="frmGood">
			...
			<input type="hidden" name="bidid" value="${good.BIDID}"/>
			<input type="hidden" name="bid_vendorid" value="${good.VENDORID}"/>
			...
			<table class="InputTable" style="border: 0px;">
				...
				<tr>
					...
					<td colspan="3">
						<input name="good_name" value="${good.GOOD_NAME}" required title="상품명" maxlength="100" type="text" style="width:300px" />
					</td>
				</tr>
				<tr>
...
<%
	for (int i=year; i>=2010; i--) {
		String reg_year = null;
		
		if (good != null) {
			reg_year = (String)good.get("GOOD_REG_YEAR");
		}
%>
	              			<option <%=StringUtils.isNotEmpty(reg_year) ? i== CommonUtils.parseInt(reg_year, year) ? "selected":"" : "" %>><%=i%></option>
<%
	}
%>
						</select>
					...
					<td colspan="3">
<%-- 						<input name="good_spec" value="${good.GOOD_SPEC}" required title="상품규격" maxlength="100" type="text" style="width:380px" ${loginUserDto.userNm eq '김철'?'':'readonly="readonly"'} /> --%>
<%-- 						<button id="btnSpec" type="button" class="btn btn-darkgray btn-xs btnSpec" style="display: ${loginUserDto.userNm eq '김철' ? ';':'none;'}">입력</button> --%>
						<input name="good_spec" value="${good.GOOD_SPEC}" required title="상품규격" maxlength="100" type="text" style="width:380px" />
						<button id="btnSpec" type="button" class="btn btn-darkgray btn-xs btnSpec" >입력</button>
						<input name="spec_spec" value="${good.SPEC_SPEC}" type="hidden" />
						<input name="spec_pi" value="${good.SPEC_PI}" type="hidden" />
						<input name="spec_width" value="${good.SPEC_WIDTH}" type="hidden" />
						<input name="spec_deep" value="${good.SPEC_DEEP}" type="hidden" />
						<input name="spec_height" value="${good.SPEC_HEIGHT}" type="hidden" />
						<input name="spec_liter" value="${good.SPEC_LITER}" type="hidden" />
						<input name="spec_ton" value="${good.SPEC_TON}" type="hidden" />
						<input name="spec_meter" value="${good.SPEC_METER}" type="hidden" />
						<input name="spec_material" value="${good.SPEC_MATERIAL}" type="hidden" />
						<input name="spec_size" value="${good.SPEC_SIZE}" type="hidden" />
						<input name="spec_color" value="${good.SPEC_COLOR}" type="hidden" />
						<input name="spec_type" value="${good.SPEC_TYPE}" type="hidden" />
						<input name="spec_weight_sum" value="${good.SPEC_WEIGHT_SUM}" type="hidden" />
						<input name="spec_weight_real" value="${good.SPEC_WEIGHT_REAL}" type="hidden" />
					</td>
				</tr>
				<tr>
...
<%
	for (Map code : productManager) {
		...
		if(good != null) {
			managerId = (String)good.get("PRODUCT_MANAGER");
			managerId = CommonUtils.nvl(managerId, loginUserId);
%>
							<option value="<%=code.get("USERID")%>" <%=code.get("USERID").equals(managerId) ? "selected":""%>><%=code.get("USERNM")%></option>
<%
		}
		else {
%>
...
%>
							<option value="<%=code.getCodeVal1()%>" <%=good != null && code.getCodeVal1().equals(GOOD_UNIT) ? "selected":""%>><%=code.getCodeNm1()%></option>
<%
	}
%>
						</select>
...
%>
				...
				<tr>
					...
					<td>
						<label><input type="radio" class="input_none" name="stock_mngt" value="Y" <%=good != null ? ("Y".equals((String)good.get("STOCK_MNGT")) ? "checked":"") : "checked"%>/> 예</label>
						<label><input type="radio" class="input_none" name="stock_mngt" value="N" <%=good != null ? ("N".equals((String)good.get("STOCK_MNGT")) ? "checked":"") : ""%>/> 아니오</label>
					</td>
					<th rowspan="2" class="check">운영사 이미지관리</th>
				...
				<tr id="add_good" <%=good != null && "Y".equals((String)good.get("ADD_GOOD")) ? "":"style='display:none'"%>>
					<th class="check">추가상품
						<div style="position:inherit; z-index:99;">
							<button id="btnAddGood" type="button" class="btn btn-primary btn-xs">추가</button>
					...
					<td colspan="5">
						<table width="100%">
							<tr>
								...
								<td>
									<input name="add_good_desc" value="${good.ADD_GOOD_DESC}" type="text" style="width:98%"/>
								</td>
							</tr>
						</table>
						<div>
...
-->
					...
					<tr>
						<th>물량배분율</th>
						<td><input id="DISTRI_RATE" name="DISTRI_RATE" number2 title="물량배분율" type="text" style="width:50px;text-align: right;" value="" /> %</td>
						<th>기본금액</th>
						<td>
					...
					<tr>
						<th class="check">진열순위</th>
						<td>
							<input id="VENDOR_PRIORITY" name="VENDOR_PRIORITY" number title="진열순위" type="text" style="width:50px;text-align: right;" value="" />
						</td>
						<th>추가금액</th>
						<td>
					...
					<tr>
						<th>
							<button id="btnDesc" type="button" class="btn btn-gray btn-xs">상품설명</button>
						</th>
						<td colspan=5>
							<div id="wrap_desc" style="height:0px;overflow:hidden">
								<textarea id="GOOD_DESC" name="GOOD_DESC" title="상세설명" style="height:200px;display:none;width: 100%;"></textarea>
							</div>
						</td>
					</tr>
					<tr>
					...
					<td rowspan="2" valign="top" class="pdl_10">
						...
						<table id="tab-Input" class="InputTable" style="border: 0px;">
							...
							<tr>
								...
								<th>기준이익율</th>
								<td>
									<input id="BUY_RATE" name="BUY_RATE" title="기준이익율" class='getSellPriceClass' number2 type="text" style="width:50px;text-align: right;" />%
									<button type="button" class="btn btn-primary btn-xs btnRateChange" alt="RATE" disabled>저장</button>
								</td>
							</tr>
							<tr>
								<th class="check">매입가</th>
								<td>
									<input id="SALE_UNIT_PRIC" name="SALE_UNIT_PRIC" class='getSellPriceClass' title="매입가" requirednumber type="text" style="width:70px;text-align: right;" readonly="readonly" />
								...
								<td>
									<input id="SELL_PRICE" name="SELL_PRICE" title="판매가" requirednumber type="text" style="width:70px;text-align: right;" />
							...
							<tr>
								...
								<th class="check">진열순위</th>
								<td>
									<input id="VENDOR_PRIORITY" name="VENDOR_PRIORITY" requirednumber title="진열순위" type="text" style="width:50px;text-align: right;" value="" />
								</td>
							</tr>
							<tr>
								<th class="check">최소주문수량</th>
								<td>
									<input id="DELI_MINI_QUAN" name="DELI_MINI_QUAN" requiredNumber title="최소주문수량" type="text" style="width:50px;text-align: right;" value="" />
								</td>
							...
							<tr>
								<th>제조사</th>
								<td>
									<input id="MAKE_COMP_NAME" name="MAKE_COMP_NAME" type="text" style="width:150px" />
								</td>
								...
								<th>상위상품코드</th>
								<td id="td_MASTER_GOOD_IDEN_NUMB"></td>
							</tr>
						</table>
					</td>
				</tr>
...
$(function(){
    ...
    $('#btnSaveGood').click(function () {
		...
		var tmpIsAdd = $('input[name=add_good]:checked').val();
		var tmpAddSubProdCnt = jq("#list1").getGridParam('reccount');
    	if(tmpIsAdd == "Y" && tmpAddSubProdCnt == 0){
    		alert("추가상품을 추가 후 저장하여 주십시오..");
    		return ;
    	}
...
//     $(document).on("blur", "input:text[requirednumber]", function() {
     ...
     **************************************/
// 	$("#SALE_UNIT_PRIC").blur(function(e) {
	$(".getSellPriceClass").keyup(function(e) {
    	if($("#BUY_RATE").val()!='') {
    		//판매가 = 100*매입가/(100-기준이익율)
			var sellPrice = 100*Number($("#SALE_UNIT_PRIC").val())/(100-Number($("#BUY_RATE").val()));
			$("#SELL_PRICE").val(Number(sellPrice));
			
			var rate = (Number($("#SELL_PRICE").val()) - Number($("#SALE_UNIT_PRIC").val())) *100 / Number($("#SELL_PRICE").val());
			if(!isNaN(rate)) $('#td_RATE').html(rate.toFixed(1) + '%');
    	}
	});
    ...
    $('.btnPriceChange').click(function (e) { // 가격변경
        ...
        if(priceType=="BUY") {
	        var row = $('#list2').jqGrid('getGridParam','selrow');
			var selRowContent = $('#list2').jqGrid('getRowData',row);
			if(selRowContent.BUY_RATE!=$("#BUY_RATE").val()) {
				alert("입력한 기준이익율을 저장되지 않았습니다. 먼저 저장하시고 매입가를 변경해 주십시오");
				return;
			}
        }
        $('#productPop').css({'width':'600px','top':'50%','left':'20%'}).html('')
        .load('/menu/product/product/popPriceChange.sys?good_iden_numb='+$('#good_iden_numb').val()+'&vendorid='+$('#VENDORID').val()+'&sell_sale_type='+priceType+'&buy_rate='+$('#BUY_RATE').val())
        .jqmShow();
    });
    ...
    $('.btnRateChange').click(function (e) { //기준이익율 저장
		if (!checkVendor(e)) return false;
		var isChangeSell = false;	//판매가 변경여부
		var sellPrice = 0;
    	if($("#BUY_RATE").val()=='') {
    		if(!confirm("기준이익율 설정을 삭제 하시겠습니까?")) return;
    	} else {
	    	//판매가 = 100*매입가/(100-기준이익율)
	    	sellPrice = 100*Number($("#SALE_UNIT_PRIC").val())/(100-Number($("#BUY_RATE").val()));
			var sellPriceString = $.number( sellPrice );
	    	if (!confirm("기준이익율["+$("#BUY_RATE").val()+"]과 기준이익율에 따른 판매가["+sellPriceString+"]를 변경 하시겠습니까?")) {
				return;
	    	}
			isChangeSell = true;
    	}
		if(!isChangeSell) if(!confirm("저장 하시겠습니까?")) return;
		var params = "good_iden_numb=" + $('#good_iden_numb').val() + '&' + $('#frmVendor').serialize();
		$.post('<%=Constances.SYSTEM_CONTEXT_PATH %>/product/updateVendorBuyRate.sys', params,
			function(msg) {
				var m = eval('(' + msg + ')');
				if (m.customResponse.success) {
					if(isChangeSell) {
						params = { 
							...
							sell_sale_type:'SALE',
							chgprice:Math.round(Number(sellPrice)),
							chg_reason:'기준이익율['+$("#BUY_RATE").val()+'] 변경에 따른 판매가 변경'
						};
				        $.post('<%=Constances.SYSTEM_CONTEXT_PATH %>/product/savePrice.sys', params,
							function(msg2) {
								var m2 = eval('(' + msg2 + ')');
								if (m2.customResponse.success) {
									alert("기준이익율과 판매가를 변경 하였습니다.");
						...
						alert("기준이익율 정보를 저장하였습니다.");
						
						$('#list2').setCell($('#VENDORID').val(), 'BUY_RATE', $("#BUY_RATE").val()==''?'&nbsp;':$("#BUY_RATE").val());
					}
				} else {
					alert('저장 중 오류가 발생했습니다.');
				}
    ...
    $('#btnSaveVendor').click(function (e) {
        ...
        if( Number($.trim($("#SELL_PRICE").val())) < Number($.trim($("#SALE_UNIT_PRIC").val())) ){
        	alert("'판매가'보다 '매입가'를 크게 입력할 수 없습니다.");
        	return;
        }
<% if(repre_good.equals("Y")) { %>
		if( Number($.trim($("#SELL_ADD").val())) < Number($.trim($("#SALE_ADD").val())) ){
			alert("'추가금액 판매가'보다 '추가금액 매입가'를 크게 입력할 수 없습니다.");
			return;
		}
<%} else {	//기준이익율 확인	%>
		if ($('#ISNEW').val()=='U') {
			...
			var selRowContent = $('#list2').jqGrid('getRowData',row);
			if(selRowContent.BUY_RATE!=$("#BUY_RATE").val()) {
				if(!confirm("기준이익율이 수정되었습니다.\n매익율을 변경 하실려면 기준이익율의 저장버튼을 사용하십시오\n기준이익율 변경없이 저장하시겠습니까?")) return;
			}
		}
<%	}	%>
        ...
        $.post('<%=Constances.SYSTEM_CONTEXT_PATH %>/product/saveVendor.sys', params,
            function(msg) {
              ...
              if (m.customResponse.success) {
                  ...
                  $('#list2').setGridParam({url:'/product/selectGoodVendorListForNew/list.sys',"postData":$('#frmGood').serializeObject()}).trigger("reloadGrid");
//                   $("#list2").setSelection($('#VENDORID').val());
              } else {
                  alert('저장 중 오류가 발생했습니다.');
              }
            }
	...
	$('.btnClose').click(function () {
		...
		if($('#list3').getGridParam('reccount')<=0) { checkUnload = true; }
		else {
			var displayChk = 0;
			for(var i=0;i<$('#list3').getGridParam('reccount');i++) {
				var rowid = $("#list3").getDataIDs()[i];	//로우 아이디 구하기
				var selrowContent = $("#list3").jqGrid('getRowData',rowid);
				if((selrowContent.DELI+selrowContent.WORK+selrowContent.BRANCH).length>0) {
					displayChk++;
				}
//	 			alert(":::"+selrowContent.DELI+":::"+selrowContent.WORK+":::"+selrowContent.BRANCH);
			}
			if(displayChk<=0) checkUnload=true;
		}
		if(checkUnload) {
			if(!confirm("상품진열 세팅이 안 되어 있습니다.\n창을 닫으시겠습니까?")) return false;
		}
		window.close();
	});
...
function funPopBigImage(){
	...
	$('#productPop').css({'width':'500px','top':'40%','left':'20%'}).html('')
    ...
    $("#list2").jqGrid({
        ...
        colModel:[
            ...
            {name:'SALE_ADD',width:70,align:'right',sorttype:'integer',formatter:'integer',
                formatoptions:{ decimalSeparator:"", thousandsSeparator:",", decimalPlaces: 0, prefix:"" },sortable:false},//매입가
            {name:'VENDORID',hidden:true, key:true},
            ...
            {name:'MASTER_GOOD_IDEN_NUMB',hidden:true},
            {name:'BUY_RATE',hidden:true}
        ],
        ...
        afterInsertRow: function(rowid, aData) {
        	if(aData.ISUSE!='1') {
        		$(this).setCell(rowid,'ISUSE','',{color:'red'});
        	}
        },
...
//                 }
            ...
            if($('#VENDORID').val()!='') {
            	...
            	$("#list2").setSelection('${not empty good.VENDORID ? good.VENDORID : param.vendorid}');
            }
        }
    });
    <% if (repre_good.equals("Y")) { %>
    ...
    $("#list3").jqGrid({
        ...
        onCellSelect: function(rowid, iCol, cellcontent, target) {
            ...
            if(colName != undefined &&colName['name']=="MNGT") {
                ...
                $('#productPop').css({'width':'600px','top':'50%','left':'10%'}).html('')
    ...
    */
    $.post(
        ...
        , function(arg) {
			...
			$("#MAKE_COMP_NAME").val(vendor.VENDORNM);
			$('#VENDORNM').change();
...
function bindVendor(vendor, opt) {
     ...
     $('#td_PRESSENTNM').html(vendor.PRESSENTNM+" / "+fnSetTelformat(vendor.PHONENUM));
     $('#td_PHONENUM').html(fnSetTelformat(vendor.PHONENUM));
     if(vendor.MASTER_GOOD_IDEN_NUMB!='' && vendor.MASTER_GOOD_IDEN_NUMB!=undefined) { 
     	$('#td_MASTER_GOOD_IDEN_NUMB').html("<a href='javascript:fntMasterGoodIdenNumb(\"" + vendor.MASTER_GOOD_IDEN_NUMB + "\", \"" + vendor.VENDORID + "\")'>" + vendor.MASTER_GOOD_IDEN_NUMB + "</a>");
     }
...
//      var rate = (Number($("#SELL_PRICE").val())-Number($("#SALE_UNIT_PRIC").val()))*100/Number($("#SELL_PRICE").val());
     if (isNaN(rate) || vendor.SELL_PRICE ==0) {
        $('#td_RATE').html('0%');
     } else {
     ...
     if (vendor.SMALL_IMG_PATH == null || vendor.SMALL_IMG_PATH == "") {
         ...
         $('#ORIGINAL_IMG_PATH').val(vendor.ORIGINAL_IMG_PATH);
...
//          $('#btnDelVen').attr('disabled', false);
 ...
 function fntMasterGoodIdenNumb(goodIdenNumb, vendorId){
	var params = 'good_iden_numb=' + goodIdenNumb;
		
	params = params + '&vendorid=' + vendorId;
	
	window.open('', 'masterGoodIdenNumb', 'width=917, height=800, scrollbars=yes, status=no, resizable=no');
	
	fnDynamicForm("/product/popProductAdm.sys", params, "masterGoodIdenNumb");
 }
 ...
 function fnPriceCallBack(chgPrice) {
	...
	var rate = (Number($("#SELL_PRICE").val())-Number($("#SALE_UNIT_PRIC").val()))*100/Number($("#SELL_PRICE").val());
	if (isNaN(rate) || Number($("#SELL_PRICE").val()) ==0) {
		$('#td_RATE').html('0%');
	} else {
		$('#td_RATE').html(rate.toFixed(1) + '%');
	}
