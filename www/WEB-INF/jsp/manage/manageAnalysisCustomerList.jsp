...
function initList() {
	jq("#list").jqGrid({
		...
		datatype:'json',
		mtype:'POST',
		colNames:['구매사명','사업자등록번호','매출금액','순위','법인Id','매출Id'],
		colModel:[
			{ name:'borgnm',index:'borgnm',width:210,align:"left",search:false,sortable:true,editable:false },//고객사명
			{ name:'businessNum',index:'businessNum',width:130,align:"center",search:false,sortable:true,editable:false },//고객사명
			{ name:'sale_requ_amou',index:'sale_requ_amou',width:150,align:"right",search:false,sortable:true
...
function fnInitProductList(clientid) {
	jq("#list2").jqGrid({
		...
		datatype:'json',
		mtype:'POST',
		colNames:['상품명','공급사명','사업자등록번호','매출금액'],
		colModel:[
			{ name:'good_name',index:'good_name',width:240,align:"left",search:false,sortable:true,editable:false },//품목명
			{ name:'vendornm',index:'vendornm',width:200,align:"left",search:false,sortable:true,editable:false , hidden:true},//공급사명
			{ name:'businessNum',index:'businessNum',width:90,align:"center",search:false,sortable:true,editable:false, hidden:true },//사업장 등록번호
			{ name:'sale_prod_amou',index:'sale_prod_amou',width:120,align:"right",search:false,sortable:true
				,editable:false,sorttype:'integer',formatter:'integer',formatoptions:{ decimalSeparator:"", thousandsSeparator:",", decimalPlaces: 0, prefix:"" }
			},//매출금액
		],
		postData: {
		...
		height:<%=listHeight2 %>,width:$(window).width()-725 + Number(gridWidthResizePlus),
		sortname:'',sortorder:'',
		caption:"상품별 매출 현황",
		viewrecords:true,emptyrecords:'Empty records',loadonce:false,shrinkToFit:false,//해당라인은 그리드 공통으로 들어가면 될거 같음(그냥 갖다 붙이 삼..)
		loadComplete:function() {},
...
function fnOnClickCustomerProductList(clientid,borgnm) {
	...
	jq("#list2").jqGrid("setCaption", "<font color='blue'>["+borgnm+"]</font>의 상품별 매출현황");
...
function exportExcel() {
	var colLabels = ['구매사명','사업자등록번호','매출금액'];	//출력컬럼명
	var colIds = ['borgnm','businessNum','sale_requ_amou'];	//출력컬럼ID
	var numColIds = ['sale_requ_amou'];	//숫자표현ID
	var figureColIds = ['businessNum'];
	var sheetTitle = "구매사별 매출 현황";	//sheet 타이틀
...
function exportExcel2() {
	...
	var figureColIds = [];
	var sheetTitle = "상품별 매출현황";	//sheet 타이틀
	var excelFileName = "analysisCustomerList";	//file명
...
function manageManual(){
	...
	//상세 매출 현황
	header = "상세 매출 현황";
	manualPath = "/img/manual/manage/manageAnalysisCustomerListManual.JPG";
	...
	<td>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr valign="top">
				...
				<td height="29" class="ptitle">상세 매출 현황
	...
	<td>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				...
				<td class="table_td_contents" width="320">
				...
				<td class="table_td_subject" width="80">고객사</td>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				<td valign="top" width="610" height="18" align="right">
					* 법인명을 클릭하시면 법인의 구매사별 매출 현황을 조회하실 수 있습니다.&nbsp;
	...
	<td>
		<table width="100%" border="0" cellpadding="0" cellspacing="0" style="height: 27px">
			<tr>
				...
				<td class="stitle">구매사별 매출 현황 그래프</td>
			</tr>
		</table>
		<table width="100%" border="0" cellpadding="0" cellspacing="0" style="height: 27px">
			<tr>
